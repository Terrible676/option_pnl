...
option.pnl <- numeric(option_num);
option.Total <- numeric(option_num);
curr_pos <- numeric(option_num);                 ## 初始化option pos,pnl,closePrice, total
curr_close <- numeric(option_num);
prePos_data <-numeric(option_num);
preClose_data <- numeric(option_num);
curr_close<-  numeric(option_num);
greeks_value <- numeric(option_num);
greeks_delta <- numeric(option_num);
greeks_gamma <- numeric(option_num);
greeks_theta <- numeric(option_num);
greeks_vega <- numeric(option_num);
greeks_rho <- numeric(option_num);
option_name <- numeric(option_num);
margin_data <- numeric(option_num);
implied_vol <- numeric(option_num);
...
for ( code_tmp in option_code){
  ...
  if(is.na(closePrice)){closePrice = 0};
...
sum_pnl <- numeric(length(trading_date));
...
for(date_tmp in trading_date){
  ...
  for (code_tmp in option_code){
    ...
    if (!is.na(frame_option[code_counter,3])){
